...
const rewardHighlights = [
  {
    ...
    emoji: "💰",
    description:
      "Most codes award cash that you can invest into new seeds, upgrades, and stronger defenders.",
  },
  {
    title: "Starter Boosts",
    emoji: "🚀",
    description:
      "Launch a fresh account with extra resources that make the opening waves far easier to clear.",
  },
  {
    title: "Event Rewards",
    emoji: "🎉",
    description:
      "Limited drops during updates or holidays often hand out premium loot or larger currency bundles.",
  },
  {
    title: "Community Bonuses",
    emoji: "🤝",
    description:
      "Developers occasionally celebrate milestones with celebratory codes for the whole player base.",
  },
];

function formatDate(input: string | null, options: Intl.DateTimeFormatOptions = { month: "short", day: "numeric", year: "numeric" }) {
  if (!input) return "—";
    ...
    <div className="container space-y-12">
      ...
      <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        {expiredCodes.length === 0 ? (
          <p className="mt-4 text-sm text-slate-300">No expired codes to report. We&apos;ll list retired promos as soon as one lapses.</p>
        ) : (
      ...
      <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          {rewardHighlights.map((perk) => (
            <article key={perk.title} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-5 text-sm text-slate-300">
              <p className="text-xl">{perk.emoji}</p>
              <p className="mt-2 text-base font-semibold text-white">{perk.title}</p>
      ...
      <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-white">Stay updated with new codes</h2>
